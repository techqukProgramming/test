...
class AccountInvoice(models.Model):
    ...
    debit_invoice_id = fields.Many2one('account.invoice', string="Invoice for which this invoice is the debit note")
    debit_invoice_ids = fields.One2many('account.invoice', 'debit_invoice_id', string='Debit Invoices', readonly=True)
    edocument_type_code = fields.Char(string='Electronic invoice type code', related='edocument_type.code', store=True)
    einvoice_hash_code = fields.Char('Hash code', copy=False)
    einvoice_cdr_zip = fields.Binary('CDR zip', copy=False)
    einvoice_sunat_responsecode = fields.Char('SUNAT Response code', copy=False)            
    ...
    einvoice_log_ids = fields.One2many('account.einvoice.log','invoice_id', string='E-invoice log',copy=False)
    einvoice_errors_sunat = ""    
    einvoice_error = fields.Text(string="Error", compute="_get_einvoice_log_ids")
    ...
    def _get_einvoice_log_ids(self):    
        for x in self:
            if x.einvoice_log_ids:
                x.einvoice_error = x.einvoice_log_ids.sorted('date',reverse=True)[0].response
        return True

    def _get_ose_supplier(self):
        if not self.journal_id.shop_id:
            raise UserError(_('Please select a Shop for the journal \'%s\'')%(self.journal_id.name,))
        if not self.shop_id.einvoice_supplier:
            raise UserError(_('Please select a OSE supplier for the company %s')%(self.company_id.name,))
        if self.shop_id and (self.shop_id.company_id != self.company_id):
            raise UserError(_('The company of the invoice (%s) is different to the Shop company (%s)')%(self.company_id.name,self.shop_id.company_id.name))
        return self.shop_id.einvoice_supplier_code
    
    @api.model
    def create(self, vals):
        if 'edocument_type' not in vals and 'journal_id' in vals:
            vals['edocument_type'] = self.env['account.journal'].browse(vals['journal_id']).edocument_type.id
        return super(AccountInvoice, self).create(vals)

    @api.multi
    def _get_invoice_values_odoofact(self):
        if not self.journal_id.edocument_type:
            raise UserError(_('Please define Edocument type on the journal related to this invoice.'))
        if not self.journal_id.edocument_type.type_of:
            raise UserError(_('Please define the \'Document type\' on the \'Edocument type\' of the journal related to this invoice.'))
        ...
        currency_exchange = self.currency_id.with_context(date=self.date_invoice)._get_conversion_rate(self.currency_id, self.company_id.currency_id,self.env.user.company_id,self.date_invoice)
        if currency_exchange == 0:
            raise UserError(_('The currency rate should be different to 0.0, Please check the rate at %s' ) % self.date_invoice) 
        send_email = ''
        ...
        values = {
            ...
            'cliente_direccion': (self.commercial_partner_id.street or '') \
                                ...
                                + (self.commercial_partner_id.district_id and ', ' + self.commercial_partner_id.district_id.name or '') \
                                + (self.commercial_partner_id.province_id and ', ' + self.commercial_partner_id.province_id.name or '') \
                                + (self.commercial_partner_id.state_id and ', ' + self.commercial_partner_id.state_id.name or '') \
                                + (self.commercial_partner_id.country_id and ', ' + self.commercial_partner_id.country_id.name or ''),
        ...
        for line in lines:
            if line.display_type == False:
                ...
                if line.igv_type.type in ['exonerado','inafecto']:
                    ...
                    igv_amount = abs(line.igv_amount) > 0.01 and abs(line.igv_amount) or 0.02
                values = {
    ...
    def api_connector_odoofact(self, invoice):        
        ...
        if response.get('errors', False):
            document = invoice and ( str(invoice.get('serie',' ')) + '-' + invoice.get('numero',' ') ) or 'No number'
            self.einvoice_errors_sunat = (self.einvoice_errors_sunat and self.einvoice_errors_sunat or '' ) + "\n Had the followed errors: \n %s, error in invoice: %s" % (response.get('errors',''),document)
        ose_accepted = False
        if response.get('errors',False):
            ose_accepted = False
        else:
        ...
        if log_id:
            self.env['account.invoice'].browse(invoice['invoice_id']).write(values_log)
    ...
    def action_invoice_send(self):
        ...
        einvoice_data = getattr(self,'_get_invoice_values_%s' % einvoice_supplier)()
        content = []
        content = getattr(self,'api_connector_%s' % einvoice_supplier)(einvoice_data)
        # Post a message when the invoice is sent
        if self.einvoice_ose_accepted:
            message = _("Electrónic document <span style='color: #21b799;'>%s</span> has sent to PSE/OSE") % (self.number)
    ...
    def action_invoice_check(self):
        ...
        if self.state == 'cancel':
            values = getattr(self,'_get_invoice_cancel_values_check_%s' % einvoice_supplier)()
        else:
            values = getattr(self,'_get_invoice_values_check_%s' % einvoice_supplier)()
        getattr(self,'api_connector_%s' % einvoice_supplier)(values)
        return True
    
    @api.model
    def cron_send_invoices(self):
        invoice_ids = self.env['account.invoice'].search([('is_einvoice','=',True),('state','not in',['draft','cancel']),('einvoice_ose_accepted','=',False),('type','in',['out_invoice','out_refund'])]).filtered(lambda inv: inv.date_invoice != False)
        for invoice in invoice_ids:
            invoice.action_invoice_send()

    @api.model
    def cron_check_invoice_ose(self):
        invoice_ids = self.env['account.invoice'].search([('is_einvoice','=',True),('state','!=','draft'),('einvoice_ose_accepted','=',True),('einvoice_sunat_accepted','=',False),('shop_id','!=',False),('type','in',['out_invoice','out_refund'])])
        for invoice in invoice_ids:
    ...
    def invoice_send_cancel(self):
        ...
        if not self.einvoice_ose_accepted:
            raise UserError(_("You need to send the invoice to SUNAT before cancel it. \n Invoice: %s-%s")% (self.einv_serie,str(self.einv_number)))
        # Cancel invoice (Odoo method)
        self.action_invoice_cancel()
        # Cancel with OSE
        values = getattr(self,'_get_invoice_cancel_values_%s' % einvoice_supplier)()   
        getattr(self,'api_connector_%s' % einvoice_supplier)(values)        
        if self.state == 'cancel':
            message = _("Invoice <span style='color: #21b799;'>%s-%s</span> nulled by SUNAT") % (self.einv_serie,str(self.einv_number))
            self.message_post(body=message)
        else:
    ...
    def debit(self, date_invoice=None, date=None, description=None, journal_id=None):
        ...
        for invoice in self:
            ...
            if invoice.type == 'out_invoice':
                message = _("This customer invoice debit note has been created from: <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a><br>Reason: %s") % (invoice.id, invoice.number, description)
            else:
                message = _("This vendor bill debit note has been created from: <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a><br>Reason: %s") % (invoice.id, invoice.number, description)
